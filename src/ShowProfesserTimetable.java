...
public class ShowProfesserTimetable extends JFrame   {
	...
	public ShowProfesserTimetable(String pID, String year, String semester) {
		super("ShowProfesserTimetable");
	...
	public void showTable(){		//테이블에 보여줄화면 정의
		...
		try {
			...
			//실제 데이터
			String query = "select * from lecture where lno in (select lno from coursehistory where pno="+pID+" and semester="+semester+" and cyear="+year+")";
			//여러개의 강의 lno 가 있을 수 있으니깐 in연산자 사용
			
			//실험 사용할거
			//String query = "select * from lecture where lno=(select lno from coursehistory where pno=4 and semester=1 and cyear=2021)";
			...
			//실제
			String query1 ="SELECT COUNT(*) FROM lecture where lno=(select lno from coursehistory where pno="+pID+" and semester="+semester+" and cyear="+year+")";
	...
	public void showTimetable() {
		...
		//d=table.getModel().getValueAt(0, 4).toString();
		
		for(int i=0;i<cnt;i++) {		//튜플 개수 만큼 바꿔야 함
			d=table.getModel().getValueAt(i, 4).toString();
			
			System.out.println(d+"55555555555555");
			//System.out.println(day[3]+"55555555555555");
