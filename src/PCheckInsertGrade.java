...
public class PCheckInsertGrade extends JFrame implements ActionListener{

	JButton btnShow,btnCancel;
	JTextField professorID, LectureID;
	...
	public PCheckInsertGrade() {
		super("PCheckInsertGrade");
	...
	public void layInit() {
		...
		LectureID = new JTextField(30);		

		professorID.setHorizontalAlignment(JLabel.CENTER);
		LectureID.setHorizontalAlignment(JLabel.CENTER);
		

		primary.add(new JLabel("교수ID"));
		primary.add(professorID);
		primary.add(new JLabel("강의ID"));
		primary.add(LectureID);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == btnShow) {
			...
			if(cnt==1) {		//pid가 존재하면
				System.out.println(cnt);
				new ShowStudenthasThatLecture(pID,lno);
			}else {		//존재하지 않으면
